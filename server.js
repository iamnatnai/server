  ...
  await usePooledConnectionAsync(async (db) => {
    ...
    db.query(query, async (err, result) => {
      if (err) {
        ...
        // add shipping cost to each product by querying the farmer's shipping cost
        for (let i = 0; i < result.length; i++) {
          ...
          if (shippingcost.length == 0) {
            result[i].shippingcost = {
              weight: 0,
              price: 0,
            };
          }
          result[i].shippingcost = shippingcost;
        }
...
//หน้าแผนที่ , หน้าสินค้าทั้งหมด , จัดการผู้ใช้งาน
//เรียงลำดับหมุด
...
//แสดงมาตรฐานสินค้า , ตั้งต่ามาตรฐาน,จัดหารผู้ใช้งาน  ,เพิ่มผู้ใช้งาน ,เพิ่มสินค้า
...
//แสดงรายละเอียดการจองสินค้า สำหรับเกษตรกร , วิเคราะห์สินค้า
...
//ตืดตามเกษตรกร
//สมาชิกติดตามเกษตรกรดูได้ในหน้าแก้ไข
...
//-------------------------------------------------------------------------------------------
//หน้าเทศกาล
//หน้าที่ทำรายละเอียดเกษตรกรต่อเทศกาลมาแสดง เกษตรกร รอการตอบกลับ
  ...
  try {
    ...
    await usePooledConnectionAsync(async (db) => {
      ...
      let query2 = `
        ...
        where ff.festival_id = ? and p.available = 1 and f.available = 1
      `;
...
//ตั้งค่าหมวดหมู่และตรามาตรฐานสินค้า
//หมวดหมู่สินค้า , สินค้าของฉัน
...
async function insertFarmer(
  ...
  return await usePooledConnectionAsync(async (db) => {
    ...
    //insert shippingcost
    await new Promise((resolve, reject) => {
      const query = `INSERT INTO shippingcost
      (farmer_id, weight, price)
        VALUES (?, 0, 0)`;
      db.query(query, [nextUserId], (err, result) => {
        if (err) {
          reject(err);
        } else {
          resolve(result);
...
//แอดมืน แก้ไขข้อมูลส่วนตัวคนอื่น
      ...
      if (product_id) {
        ...
        JSON.parse(certificate).forEach(async (cert, index) => {
          let certAlreadyExist = await new Promise((resolve, reject) => {
            db.query(
              `SELECT * FROM certificate_link_farmer WHERE standard_id = ? and product_id = ? and farmer_id = ? and status not like "reject"`,
...
//ดูประวัติการซื้อขายในฝั่งเกษตรกร
